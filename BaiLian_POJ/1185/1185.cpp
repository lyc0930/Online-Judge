...
int main()
{
    ...
    //产生的相邻的 1 相隔 2 位以上的2进制数，在 10 位数范围内共有 60 个
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0, cnt = 0; j < Boundary; j++, cnt = 0)
        {
            ...
            {
                if (((1 << k) & bit_Table[j])) //如果 bit_Table[j] 的第 k+1 位是 1
                    cnt++;
                if ((NOT(Map[i][k]))AND(((1 << (width - k - 1)) & bit_Table[j]))) //如果 bit_Table[j] 的第 width-k 位是 1, 且 Map[i][k] 是高地
                {
                    cnt = -1;
                    break;
                }
